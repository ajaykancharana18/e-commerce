...
const RelatedProducts = () => {
  return (
    <div>
        <h3 className='font-bold text-4xl p-4 text-center text-tertiary '>Related Products</h3>
        <hr className=''/>
                ...
                POPULAR.map((popular)=>(
                    ...
                    <div key={popular.id} className=' shadow-xl transition-all duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 rounded-lg'>
                        <div className='flex justify-center items-center'>
                            <Link to={`${popular.id}`} > <img className='  rounded-t-2xl' src={popular.image} alt="" /> </Link>
                        </div>
